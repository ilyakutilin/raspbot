...
async def _check_last_station_db_update(
    ...
    async with async_session_factory() as session:
        query = await session.execute(select(func.max(LastUpdatedORM.updated)))
        ...
        if current_time_diff > max_time_diff:
            logger.info(
                f"It was more than {days_between_updates} days since the stations DB "
                "was last updated. Therefore, starting DB population process now."
            )
            await main()
        logger.info(
...
async def start_update_monitoring():
    """Starts the update monitoring."""
    scheduler = AsyncIOScheduler()
    scheduler.add_job(
        _check_last_station_db_update, "cron", hour=3, id="check_last_station_db_update"
    )
